...
for tkn in tokenized_story:
    if tkn not in tokenized_vocabulary:
...
def spell_check(vocabulary_file,text_file,special_characters=[",",".","'",";","\n"]):
    misspelled_words=[]
    vf=open(vocabulary_file,"r").read()
    tf=open(text_file,"r").read()
    tokenized_volcabulary=tokenize(vf,special_characters)
    tokenized_text=tokenize(tf,special_characters,clean=True)
    for tkns in tokenized_text:
        if (tkns not in tokenized_vocabulary) and tkns!='':
            print(tkns)
            misspelled_words.append(tkns)
